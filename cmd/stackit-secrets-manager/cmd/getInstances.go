...
func printInstances(instances []api.Instance) error {
	...
	defer writer.Flush()
	_, err := fmt.Fprintf(writer, "Id\tName\tUsers\tSecrets\tVersions\tAPI URL\tSecrets Engine\tState\n")
	if err != nil {
		return fmt.Errorf("failed to write to tabwriter: %w", err)
	}
	for _, instance := range instances {
		_, err = fmt.Fprintf(writer, "%s\t%s\t%d\t%d\t%d\t%s\t%s\t%s\n", instance.Id, instance.Name, instance.UserLimit, instance.SecretLimit, instance.VersionLimit, instance.ApiUrl, instance.SecretsEngine, instance.State)
		if err != nil {
			return fmt.Errorf("failed to write to tabwriter: %w", err)
		}
	}
